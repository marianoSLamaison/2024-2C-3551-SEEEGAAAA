...
namespace Utils
{
    ...
    {
        public static Vector2 RandomTilt(Vector2 vec, float rotIz, float rotDe, Random RNG)
        {
            float ranNum = RNG.NextSingle();
            float v = ranNum * (rotDe - rotIz) + (1f - ranNum) * rotIz;
            return Vector2.Transform(vec, Matrix.CreateRotationZ(v));
        }
